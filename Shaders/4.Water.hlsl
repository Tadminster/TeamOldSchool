...
float4 PS(PixelInput input) : SV_TARGET
{
	...
	if (Opacity < 1.0f)
	{
		...
		float3 refractionViewDir = normalize(input.wPosition.xyz - refractionCam.xyz);
		float3 refraction = refract(ViewDir, Normal , 1.0f);
		refraction.xyz += MappingNormal.x * 0.1f;
		float3 refractionRGB = WaterRefraction.Sample(WaterRefractionSampler, refraction.xyz).xyz;
